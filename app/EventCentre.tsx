...
export default function EventCentre() {
  ...
  const attendeePicker = <div className="event-wide event-attendee-picker"><label>Who should attend?<select value={mode} onChange={e => setMode(e.target.value)}><option value="senior">Senior Section</option><option value="junior">Junior Section</option><option value="all">Both sections</option><option value="selected_members">Choose members</option><option value="nco_council">NCO Council</option></select></label>{mode === "selected_members" && <><label>Search members<input value={search} onChange={e => setSearch(e.target.value)} placeholder="Name or section" /></label><p>{selected.length} selected</p><div className="event-member-options">{members.filter(m => (m.name+" "+m.section).toLowerCase().includes(search.toLowerCase())).map(m => <label key={m.id}><input type="checkbox" checked={selected.includes(m.id)} onChange={e => setSelected(ids => e.target.checked ? [...ids,m.id] : ids.filter(id => id !== m.id))} /><span>{m.name}<small>{m.section} Section</small></span></label>)}</div></>}</div>;
