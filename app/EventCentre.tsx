...
export default function EventCentre() {
  ...
  const attendeePicker = <div className="event-wide event-attendee-picker"><label>Who should attend?<select value={mode} onChange={e => { setMode(e.target.value); if (e.target.value !== "selected_members") setSelected([]); }}><option value="senior">Senior Section</option><option value="junior">Junior Section</option><option value="all">Both sections</option><option value="selected_members">Choose specific members</option><option value="nco_council">NCO Council</option></select></label>{mode === "selected_members" && <><label>Search members<input value={search} onChange={e => setSearch(e.target.value)} placeholder="Name or section" /></label><div className="event-attendee-summary"><strong>{selected.length} member{selected.length === 1 ? "" : "s"} selected</strong>{selected.length > 0 && <button type="button" className="text-button" onClick={() => setSelected([])}>Clear selection</button>}</div><div className="event-member-options">{members.filter(m => (m.name+" "+m.section).toLowerCase().includes(search.toLowerCase())).map(m => <label key={m.id}><input type="checkbox" checked={selected.includes(m.id)} onChange={e => setSelected(ids => e.target.checked ? (ids.includes(m.id) ? ids : [...ids,m.id]) : ids.filter(id => id !== m.id))} /><span>{m.name}<small>{m.section === "senior" ? "Senior" : "Junior"} Section</small></span></label>)}{members.length === 0 && <p>No members are available to choose.</p>}</div></>}</div>;
