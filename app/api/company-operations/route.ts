...
export async function GET(request: Request) {
  ...
  const allMembers = await runtime.DB.prepare("SELECT id, name, rank, section, squad, email, band_member, organisation FROM members WHERE section IN ('senior', 'junior') AND COALESCE(organisation,'BB')='BB' ORDER BY name").all<{ id: number; name: string; rank: string; section: string; squad: string; email: string; band_member: number; organisation: string }>();
  ...
  if (workspace === "duties") {
    ...
    const assignments = await runtime.DB.prepare(`SELECT d.*, dt.name AS duty_name, m.name AS member_name, e.title AS event_title
      FROM duty_assignments d JOIN duty_types dt ON dt.id=d.duty_type_id LEFT JOIN members m ON m.id=d.member_id LEFT JOIN company_events e ON e.id=d.event_id ORDER BY d.starts_at DESC`).all<{ section: string; squad: string }>();
    const visibleAssignments = user.role === "member" ? assignments.results.filter((assignment) => Number((assignment as { member_id: number | null }).member_id) === own?.id) : scoped(user, assignments.results);
    return Response.json({ module: workspace, permissions, members, events: visibleEvents, dutyTypes: dutyTypes.results, assignments: visibleAssignments });
  }
  ...
  if (workspace === "band") {
    ...
    const [profiles, instruments, history, rehearsals, performances, assessments] = await Promise.all([
      runtime.DB.prepare("SELECT bp.*,m.name,m.rank,m.section,m.squad,COALESCE(m.organisation,'BB') AS organisation FROM band_profiles bp JOIN members m ON m.id=bp.member_id ORDER BY m.name").all(),
    ...
    const bandParticipants = [...members.filter((m) => m.band_member).map((m) => ({ ...m, organisation: "BB" })), ...(gbMembers.results as Array<Record<string, unknown>>).filter((m) => Number(m.band_member) === 1).map((m) => ({ ...m, organisation: "GB" }))];
...
export async function POST(request: Request) {
  ...
  if (action === "update_duty_availability") {
    const id=number(body.assignmentId); const assignment=await runtime.DB.prepare("SELECT d.*,m.email FROM duty_assignments d LEFT JOIN members m ON m.id=d.member_id WHERE d.id=?").bind(id).first<{member_id:number|null;email:string|null;section:string;squad:string}>();
    const ownAssignment=Boolean(assignment?.email&&assignment.email.toLowerCase()===user.email.toLowerCase()); if(!assignment||(!ownAssignment&&!canManageScopedPermission(user,"programme.rosters.manage",assignment.section,assignment.squad)))return Response.json({error:"Duty assignment not available"},{status:404});
    const available=body.available!==false; await runtime.DB.prepare("UPDATE duty_assignments SET status=?,availability_note=?,updated_at=? WHERE id=?").bind(available?"assigned":"unavailable",text(body.availabilityNote,500),now,id).run();
    if(!available){const recipients=await activeUserIdsForRolesOrPermission(["admin","officer"],"programme.rosters.manage");await actionItem({key:`duty-unavailable:${id}`,rule:"duty_rosters",recipients,title:"Duty needs a substitute",description:text(body.availabilityNote,500)||"The assigned member reported that they are unavailable.",url:"/?open=duties",sourceType:"duty_assignment",sourceId:String(id),priority:"important"});}else await resolveActions(`duty-unavailable:${id}`);
  ...
  if (action === "submit_leave") {
    ...
    const attachmentId=number(body.attachmentDocumentId)||null; if(attachmentId){const document=await runtime.DB.prepare("SELECT member_id FROM member_documents WHERE id=?").bind(attachmentId).first<{member_id:number}>();if(!document||document.member_id!==member.id)return Response.json({error:"Supporting document is not available for this member"},{status:404});}
    try { const result=await runtime.DB.prepare("INSERT INTO leave_requests (event_id,member_id,reason,attachment_document_id,status,submitted_by_user_id,created_at,updated_at) VALUES (?,?,?,?,'pending_squad',?,?,?)").bind(number(body.eventId),member.id,text(body.reason),attachmentId,user.id,now,now).run(); const id=Number(result.meta.last_row_id); const reviewers=await runtime.DB.prepare("SELECT id FROM users WHERE active=1 AND account_status='active' AND role IN ('nco','squad_leader') AND squad=?").bind(member.squad).all<{id:number}>(); await actionItem({key:`leave:${id}`,rule:"leave_requests",recipients:reviewers.results.map(r=>r.id),title:"Leave request needs squad review",description:`${member.name} submitted an event leave request.`,url:"/?open=leave",sourceType:"leave_request",sourceId:String(id)}); return Response.json({ok:true,id,message:"Leave request submitted."}); } catch { return Response.json({error:"A leave request for this event already exists"},{status:409}); }
  }
  ...
  if (action === "submit_service_hours") { const memberId=number(body.memberId)||own?.id||0; const member=await memberById(memberId); if(!member||!canViewMemberScope(user,member))return Response.json({error:"Member not available"},{status:404}); const minutes=Math.round(number(body.hours)*60+number(body.minutes)); if(minutes<=0||minutes>24*60)return Response.json({error:"Enter a valid service duration"},{status:400}); const evidenceId=number(body.evidenceDocumentId)||null;if(evidenceId){const document=await runtime.DB.prepare("SELECT member_id FROM member_documents WHERE id=?").bind(evidenceId).first<{member_id:number}>();if(!document||document.member_id!==member.id)return Response.json({error:"Evidence document is not available for this member"},{status:404});} const result=await runtime.DB.prepare("INSERT INTO service_hour_submissions (member_id,activity,service_date,duration_minutes,category,description,evidence_document_id,status,submitted_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,'pending_squad',?,?,?)").bind(member.id,text(body.activity,200),text(body.serviceDate,10),minutes,text(body.category,80),text(body.description),evidenceId,user.id,now,now).run(); const id=Number(result.meta.last_row_id); const reviewers=await runtime.DB.prepare("SELECT id FROM users WHERE active=1 AND role IN ('nco','squad_leader') AND squad=?").bind(member.squad).all<{id:number}>(); await actionItem({key:`service:${id}`,rule:"service_verification",recipients:reviewers.results.map(r=>r.id),title:"Service hours need squad confirmation",description:`${member.name} submitted ${Math.round(minutes/6)/10} service hours.`,url:"/?open=service",sourceType:"service_hours",sourceId:String(id)}); return Response.json({ok:true,id,message:"Service hours submitted for squad confirmation."}); }
  ...
  if (action === "archive_gb_band_member") {
    if (!hasPermission(user,"band.manage_profiles")) return Response.json({error:"Band profile permission required"},{status:403});
    const id=number(body.memberId); await runtime.DB.prepare("UPDATE members SET band_member=0 WHERE id=? AND COALESCE(organisation,'BB')='GB'").bind(id).run(); await runtime.DB.prepare("UPDATE band_profiles SET active=0,updated_by_user_id=?,updated_at=? WHERE member_id=?").bind(user.id,now,id).run(); return Response.json({ok:true,message:"GB band member archived; history was preserved."});
  }
  if (action === "save_band_profile") { if(!hasPermission(user,"band.manage_profiles"))return Response.json({error:"Band profile permission required"},{status:403}); const member=await runtime.DB.prepare("SELECT id,organisation FROM members WHERE id=? AND (COALESCE(organisation,'BB')='BB' OR COALESCE(organisation,'BB')='GB')").bind(number(body.memberId)).first<{id:number;organisation:string}>(); if(!member)return Response.json({error:"Band participant not found"},{status:404}); await runtime.DB.prepare("INSERT INTO band_profiles (member_id,instrument_section,proficiency,position,active,notes,updated_by_user_id,updated_at) VALUES (?,?,?,?,?,?,?,?) ON CONFLICT(member_id) DO UPDATE SET instrument_section=excluded.instrument_section,proficiency=excluded.proficiency,position=excluded.position,active=excluded.active,notes=excluded.notes,updated_by_user_id=excluded.updated_by_user_id,updated_at=excluded.updated_at").bind(member.id,text(body.instrumentSection,80),text(body.proficiency,80),text(body.position,80),body.active===false?0:1,text(body.notes),user.id,now).run(); return Response.json({ok:true,message:"Band profile saved."}); }
