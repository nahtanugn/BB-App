  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {view === "members" && <section className="member-grid">{filteredMembers.map((member) => { const stats = memberStats(member); const readiness = presidentReadiness(member); return <article className="member-card" key={member.id}><div className="member-card-top"><div className="avatar large">{initials(member.name)}</div><div className="card-actions"><button className="edit-member" aria-label={`Edit ${member.name}`} onClick={() => openEditMember(member)}>Edit</button><button className="more" aria-label={`Remove ${member.name}`} onClick={() => deleteMember(member)}>×</button></div></div><h2>{member.name}</h2><p>{member.rank} · Joined {member.joined_at}</p><div className="member-numbers"><div><strong>{stats.awarded}</strong><span>Awards</span></div><div><strong>{stats.active}</strong><span>Active</span></div><div><strong>{member.service_years}</strong><span>Years</span></div></div><div className="readiness"><div><span>President’s Award</span><strong>{readiness.complete}/{readiness.total}</strong></div><div className="progress-track"><div style={{ width: `${readiness.percent}%` }} /></div></div></article>; })}</section>}
