...
test("the distribution contains generic deployment configuration and no tracked operational data", async () => {
  ...
  const oneClick = await readFile(new URL("../wrangler.jsonc", import.meta.url), "utf8");
  ...
  assert.match(config, /wrangler\.jsonc/);
  assert.match(migrations, /00000000-0000-0000-0000-000000000000/);
  assert.match(oneClick, /"binding": "DB"/);
  assert.doesNotMatch(oneClick, /database_id/);
  assert.match(packageJson, /wrangler d1 migrations apply DB --remote --config wrangler\.jsonc/);
  assert.match(packageJson, /"SETUP_TOKEN"/);
  assert.match(readme, /deploy\.workers\.cloudflare\.com/);
  ...
  assert.match(deploymentConfig, /relative\(dirname\(output\), resolve\("worker\/index\.ts"\)\)/);
  assert.match(deploymentConfig, /migrations_dir: migrationsDir/);
  assert.match(deploymentConfig, /process\.env\.ADMIN_EMAIL/);
