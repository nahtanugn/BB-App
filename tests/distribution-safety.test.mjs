...
test("the distribution contains generic deployment configuration and no tracked operational data", async () => {
  ...
  assert.match(tauriCapability, /nahtanugn\.github\.io/);
  ...
  assert.match(setup, /Start guided setup/);
  assert.match(setup, /Help &amp; tutorials/);
  assert.match(setup, /nahtanugn\.github\.io\/BB-App\/setup/);
  assert.doesNotMatch(setup, /Administrator setup guide|ADMIN_EMAIL|SETUP_TOKEN|new-company/);
